...
!
      ...
      DOUBLE PRECISION GPRDTIME_,LPRDTIME,RESTE
!
...
!
!=======================================================================
! COMPUTES THE MAXIMUM SPEED supp Cerema AND ASSOCIATED TIME
!=======================================================================
!
...
!
!=======================================================================
! COMPUTES THE MAXIMUM DISCHARGE TMAXV est QMAX...
!=======================================================================
!
      IF(SORLEO(30).OR.SORIMP(30)) THEN
!        IF(.NOT.DEJA2_PRERES) THEN
		IF(.NOT.DEJA3_PRERES) THEN
!          CALL OS('X=C     ',X=MAXV ,C=0.D0)
!          CALL OS('X=C     ',X=TMAXV,C=AT)
		  CALL OS('X=C     ',X=TMAXV ,C=0.D0)
!		  DEJA2_PRERES=.TRUE.
          DEJA3_PRERES=.TRUE.
        ELSE
          DO N=1,NPOIN
!            XMAX=SQRT(U%R(N)**2+V%R(N)**2)
			XMAX=H%R(N)*SQRT(U%R(N)**2+V%R(N)**2)
!           DRY LAND EXCLUDED (TO AVOID RANDOM TIMES)
!            IF(XMAX.GT.MAXV%R(N).AND.H%R(N).GT.0.01D0) THEN
			IF(XMAX.GT.TMAXV%R(N).AND.XMAX.GT.0.001D0) THEN
              TMAXV%R(N)=XMAX
!              IF(SORLEO(30).OR.SORIMP(30)) TMAXV%R(N)=AT
            ENDIF
          ENDDO
