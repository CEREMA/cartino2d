...
!+        First version
!
!history  N.HOCINI, F.PONS (CEREMA MED)
!+        09/11/2022
!+        V8P3
!+   READING OF SPATIAL RAINFALL FILE
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
!
!
      INTEGER I,AA,UL     ! N.HOCINI : AA LOOP ON 1:NPOIN
...
!
	  ...
	  DOUBLE PRECISION RFM_S(NPOIN) ! N.HOCINI : RFM_S RAINFALL VECTOR OF NPOIN 
	  DOUBLE PRECISION, DIMENSION(:), ALLOCATABLE, SAVE :: MM_AT2_S ! N.HOCINI : VECTOR OF RAINFALL READ FROM FILE AT AT2
!
!
!      
      SAVE AT1,AT2 ! N.HOCINI : SAVING AT1 AND AT2
...
!           FORMATTED DATA FILE
!     +> 4: RAINFALL DEFINED AS A SPACE-TIME MATRIX READ IN A
!           FORMATTED DATA FILE
!+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
!
...
!
        ENDIF
      ELSEIF(RAINDEF.EQ.4) THEN  ! N.HOCINI : OPTION 4 FOR RAINDEF SPATIAL RAINFALL FILE
!		CALLING RAIN_INTERPOL SUBROUTINE AT FIRST TIMESTEP 		
        IF(AT.EQ.DT) THEN
          ALLOCATE(MM_AT2_S(NPOIN))
	      CALL RAIN_INTERPOL(NPOIN,MM_AT2_S,AT1,AT2,FILES)		
	    ENDIF
!
        IF(AT.LT.AT1) THEN
          WRITE(LU,*)' '
...
11      CONTINUE
        IF(AT.GE.AT1.AND.AT.LE.AT2) THEN
!         RAINFALL AT TIME AT OVER ONE TIME-STEP RFM_S, M
          RFM_S = (MM_AT2_S / 1000.D0 / MAX((AT2-AT1),EPS)) * DT
        ELSE
		  AT1=AT2	
		  CALL RAIN_INTERPOL(NPOIN,MM_AT2_S,AT1,AT2,FILES)		
          GO TO 11
...
!     ACCRF = ACCRF + RFM
      IF(RAINDEF.EQ.4) THEN
	    CALL OV('X=X+Y   ', X=ACCR%R, Y=RFM_S, DIM1=NPOIN)
	  ELSE
