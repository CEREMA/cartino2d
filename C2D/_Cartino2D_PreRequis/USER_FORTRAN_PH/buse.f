...
!FPBUG  INITIALISATION DE Q A 0
!FPBUG	exemple de bug, parfois l'ouvrage n+1 n'est pas dans les conditions de if... et le débit de l'ouvrage précédent est repris...
		Q=0.D0
!
...
!
        IF(I1.GT.0) THEN
          S1=H(I1)+ZF(I1)
          QMAX1=0.9D0*H(I1)*V2DPAR%R(I1)*3/DT
        ELSE
          S1=0.D0
          QMAX1=0.D0
        ENDIF
        IF(I2.GT.0) THEN
          S2=H(I2)+ZF(I2)
          QMAX2=0.9D0*H(I2)*V2DPAR%R(I2)*2/DT
        ELSE
          S2=0.D0
          QMAX2=0.D0
        ENDIF
...
!               PRESSURE FLOW --> ORIFICE LAW
                ...
                ELSE
                  SECBUS(N) = LARG*HAUT1
                ENDIF
! MDU               IF(S1.GE.(RD1+H1).AND.S2.LT.(RD2+H2)) THEN
                 IF(S1.GE.(RD1+H1).AND.S2.LT.(RD2+H2)
     &                .AND.S1.GT.(RD2+H2)) THEN
...
!               PRESSURE FLOW --> ORIFICE LAW
                ...
                ELSE
                  SECBUS(N) = LARG*HAUT1
                ENDIF
!MDU            IF(S2.GE.(RD2+H2).AND.S1.LT.(RD1+H1)THEN
!MDU  BUG  CAR a aucun moment verif si S2>(RD1+H1) voir pareil dessus
                IF(S2.GE.(RD2+H2).AND.S1.LT.(RD1+H1)
     &                .AND.S2.GT.(RD1+H1)) THEN
...
!               SUBMERGED WEIR - FLOW TYPE 3
                IF(S2.GT.(TWOTHIRDS*(S1-RD1)+RD2)) THEN
				  ...
				  IF(ENTET) THEN
				     WRITE(LU,*) 'CULVERT ',N,' S1>S2 FSF - TYPE 3'
				  ENDIF
!               UNSUBMERGED WEIR - FLOW TYPE 2
     ...
     &                     /(CE1+L+CS2+TRASH))
				  IF(ENTET) THEN
					WRITE(LU,*) 'CULVERT ',N,' S1>S2 FSF - TYPE 2'
				  ENDIF
                ENDIF
!
!             PRESSURE FLOW --> ORIFICE LAW
...
!               FLOW TYPE 6
     ...
     &               *SQRT(2.D0*GRAV*(S1-(RD2+HAUT))/(CE1+L+CS2+TRASH))
				  IF(ENTET) THEN
				    WRITE(LU,*) 'CULVERT ',N,' S1>S2 - PF TYPE 6'	 
				  ENDIF
!             FLOW TYPE 5
                ELSEIF(LONG.LT.CORR56*HAUT1) THEN
				  ...
				  IF(ENTET) THEN
				    WRITE(LU,*) 'CULVERT ',N,' S1>S2 - PF TYPE 5'
				  ENDIF
                ENDIF
!           FLOW TYPE 4 SUBMERGED OUTLET
              ELSEIF(S1.GT.(RD1+H1).AND.S2.GT.(RD2+H2)) THEN
                ...
                Q = SECBUS(N)*SQRT(2.D0*GRAV*(S1-S2)/(CE1+L+CS2+TRASH))
				IF(ENTET) THEN
				  WRITE(LU,*) 'CULVERT ',N,' S1>S2 - PF TYPE 4'
				ENDIF
              ENDIF
...
!
        IF(DBUS(N).GT.0.D0) THEN
			IF(ENTET.AND.DBUS(N).GT.QMAX1) THEN
				WRITE(LU,*) 'CULVERT ',N,' LIMITATION DEBIT ENTREE'
			ENDIF
