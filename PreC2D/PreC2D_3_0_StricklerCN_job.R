StricklerCN=function(Zones,iZone,Departement)
{
  ...
  #### Pour gérer sique table assemblage
  opt_BDtopoOCSGE=1
  faisle=1
  
  
  NeFaispas=0 # ne pas toucher
    ...
    for (iDpt in 1:dim(Dpt)[1])
    {
      ...
      if (NeFaispas==0)
      {
        ...
        if( length(indOCS_GE)>0)
        {
          ...
          # dsnlayerStriCN_=file.path(dsnlayer,sous_doss,SousDossUser,nom_Param[1])
          nom_Param_=nom_Param[1]
            ...
            for (ifich in nomfichier)
            {
              ...
              if (type_geom=="Ligne") 
              {
                ...
                if (Nada==0)
                {
                  ...
                  nom_tmp=file.path(dsnlayerStriCN_,paste0("Contour",Zone$ZONE,"_",i,"_tmp.gpkg"))
              ...
              if (type_geom=="Polygone")
              {
                ...
                # browser()
                #### carte scole Strickler
                # lié au bug de 
                # ERREUR : Found unclosed outer polygon ring, can be closed when cleaning
                # polygons is not disabled
                # lié à la BDTopo IGN
                # SurfEau <- st_read(ifich, layer = coucheGPKG, wkt_filter = bbox_wkt, query =  paste0('SELECT * FROM ', shQuote(coucheGPKG), ' WHERE ', champ, ' ', valChamp))
                # Nom_tmp_CS_Strickler=file.path(dsnlayerStriCN_,paste0(NomG,"_",Zone$ZONE,".gpkg"))
                # st_write(st_buffer(SurfEau[,1],0.01),Nom_tmp_CS_Strickler, delete_dsn = TRUE)
                # cmd=paste0("v.in.ogr -o -r --quiet --overwrite input=",Nom_tmp_CS_Strickler," snap=1"," output=",NomG)
                # cmd=paste0("v.in.ogr -o -r --overwrite input=",Nom_tmp_CS_Strickler," snap=1"," output=",NomG)
                # print(cmd);toto=system(paste0(BatGRASS," ",SecteurGRASS," --exec ",cmd))
                # browser()
                # unlink(Nom_tmp_CS_Strickler)
