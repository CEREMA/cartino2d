...
nomlayerC= "Cartino2D_Secteur_20251126_170525.gpkg"
...
RelSce_OHObstrue=rbind(
  cbind("SceMaxMedzzz","Eext"),
  cbind("Sce20141006zzz","Emoy"),
  cbind("Sce20140929zzz","Emoy")
)
...
################################################################################
# --------------- OH locaux ----------------------------------------------------
nomOHlocaux="C:\\Cartino2D_Github\\France\\_OuvragesHydrauliques\\OH_MTPCLL_20240711.gpkg"
BufOHlocaux=5
# --------------- Ponts Isidor ---------------------------------------------------
...
dsnLidar="H:\\FILINO_Travail\\06_MNTTIN_Direct"
dsnLidar="C:\\AFFAIRES\\CNIR_Mosaique\\TA\\MNT_Mosaique"
nomTA="TA_Reso005.gpkg"
nomTA="TA_1_2_3_4_5_mosaique.gpkg"
...
ResoStrCN=c(1,5,25)
# Dossier de travail
dsnlayerStriCN = "EAIM" #Si ce n'est pas un chemin complet (recommandé),
# il mettra les données par défaut dans un répertoire de dsnlayer
# Vous pouvez aussi imposer une sortie comme C:\\Cartino2D_Github\\France\\_Strickler\\P4UFOCS"
# fichier de relation CN/Strickler/BDD (voir exemples)
nomXLS = file.path(chem_routine,"PreC2D","Relation_CN_Str_20241106_CNIR_BDTopo_RPG_CLC_Station.xlsx")
# nomXLS = file.path(chem_routine,"PreC2D","Relation_CN_Str_20241106_CNIR_BDTopo_RPG_CLC_Station_Etranger.xlsx")
nomXLS = file.path(chem_routine,"PreC2D","Relation_Str_20250419_EAIM_BDTopo_Station.xlsx")
# Nom des colonnes à exporter dans le fichier xls
nom_Param=cbind("CN_C")#,"Strickler")#,"CN_B""Strickler",
nom_Param=cbind("Strickler")
################################################################################
...
DateDebut=201404030000  # 2021 Durée exacte
DateFin=  201404100000
use_debu_fin=TRUE
AMC = 2
...
duree <- 24  # Durée totale en heures
# duree <- 12  # Durée totale en heures
nomdpt=cbind("11")#,"13","30","34","66","74","76")
# tpic <- 12 # Heure du pic de pluie
tpic <- 8 # Heure du pic de pluie
Periode_retour <- c(100) #c(100,500) Période de retour ==> 1000, 500, 100, 50, 20, 10, 5, 2
...
################################################################################
###################### PreC2D_4_4_ReseauSHYREG_Debits ##########################
################################################################################

dsnlayerQ="H:\\Debits\\Shyreg_Debit" # Lien vers le dossier contenant les données SHYREG de débits de pointe sur la France
# nomZoneCEB="C:\\Cartino2D_Github\\France\\Cartino2D_Secteur_20240923_165346.gpkg" # LIen vers les Secteurs débits formés à l'étape PreC2D_1_1_Zone_DPE
TA_Lidar="F:\\LidarHD_MNTIGN_V_0_1\\LidarHD_MNTIGN_V_0_1.gpkg"
...
nomSH="G:\\Sandre\\EAIM_PtspourSC_Cerema.gpkg"
# nomSH="G:\\Sandre\\BUG.gpkg"
